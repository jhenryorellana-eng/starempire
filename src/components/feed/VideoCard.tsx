...
export function VideoCard({ item, isActive, shouldPreload, shouldEagerPreload, hasBottomNav }: VideoCardProps) {
  ...
  const [showInspireAnim, setShowInspireAnim] = useState(false);
  ...
  const handleTap = useCallback(() => {
    ...
    if (timeSinceLastTap < 300) {
      // Double tap → "Me inspiró"
      handleReact('inspired');
      setShowInspireAnim(true);
      setTimeout(() => setShowInspireAnim(false), 800);
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        setFeedItem((prev) => ({
          ...
          hasGameChanged: type === 'game_changer' ? data.active : prev.hasGameChanged,
          inspiredCount: type === 'inspired'
            ? prev.inspiredCount + (data.active ? 1 : -1)
            : prev.inspiredCount,
          gameChangerCount: type === 'game_changer'
            ? prev.gameChangerCount + (data.active ? 1 : -1)
            : prev.gameChangerCount,
        }));
      }
    } catch (err) {
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: `${feedItem.companyTitle} - Lección ${feedItem.lessonNumber}`,
  ...
  const handleViewCompany = () => {
    router.push(`/explorar/empresa/${feedItem.companyId}?fromLesson=${feedItem.id}`);
  };
