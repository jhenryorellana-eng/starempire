...
export async function GET(
  ...
  try {
    ...
    const { data: items } = await supabase
      .from('playlist_items')
      .select(`*, lessons (*, companies (title, founder))`)
    ...
    const formattedItems = (items || []).map((item: any) => ({
      ...
      lesson: item.lessons ? {
        ...
        keyLesson: item.lessons.key_lesson,
      ...
      companyTitle: item.lessons?.companies?.title,
      companyFounder: item.lessons?.companies?.founder,
    }));
