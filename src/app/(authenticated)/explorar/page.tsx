...
import type { Company, Lesson } from '@/types';

export default function ExplorarPage() {
  const { token } = useAuthStore();
  const [trendingCompanies, setTrendingCompanies] = useState<Company[]>([]);
  const [mostSavedLessons, setMostSavedLessons] = useState<(Lesson & { companyTitle: string; companyFounder: string })[]>([]);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [trendingRes, savedRes, historyRes] = await Promise.all([
        fetch('/api/companies?sort=trending&limit=10', { headers }),
    ...
    <div className="min-h-screen bg-background-dark">
      ...
      <div className="px-5 py-6 space-y-8 pb-24">
        ...
        {searchResults !== null ? (
          ...
          <>
            {/* Trending */}
            <TrendingCarousel companies={trendingCompanies} />
            ...
            {mostSavedLessons.length > 0 && (
              <section>
                <div className="flex items-center justify-between mb-4 px-1">
                  <h2 className="text-lg font-bold text-white">Lecciones más guardadas</h2>
                </div>
