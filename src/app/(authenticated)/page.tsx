...
export default function FeedPage() {
  ...
  const companyCompleteRef = useRef(false);
  ...
  const loadFeed = useCallback(async (mode: string, extra?: Record<string, string>) => {
    ...
    try {
      ...
      // Send exclude IDs (skip for company mode)
      if (mode !== 'company' && shownIdsRef.current.size > 0) {
      ...
      if (res.ok) {
        ...
        if (data.companyComplete) {
          companyCompleteRef.current = true;
        }

        setVideos((prev) => {
          // For initial load or company mode, replace
          if (prev.length === 0 || mode === 'company') return data.items;
  ...
  useEffect(() => {
    if (lessonId) {
      // Company mode: load company lessons starting from this lesson
      loadFeed('company', { startFromLesson: lessonId });
    } else if (urlMode === 'bookmarks') {
      // Bookmarks mode from coleccion
      loadFeed('bookmarks');
    } else {
      loadFeed('normal');
    }
  ...
  // Scroll to specific lesson when loaded (company mode or bookmarks startId)
  useEffect(() => {
  ...
  const loadMore = useCallback(() => {
    ...
    if (urlMode === 'bookmarks') {
      loadFeed('bookmarks');
    } else if (companyCompleteRef.current || !lessonId) {
      // Normal mode (or company finished → switch to normal)
      loadFeed('normal');
    }
  }, [loadFeed, lessonId, urlMode]);
  ...
  if (videos.length === 0) {
    return (
      <div className="fixed inset-0 bg-app-black flex items-center justify-center p-8">
        <div className="text-center">
          <span className="material-icons-round text-6xl text-gray-600 mb-4 block">domain</span>
          <h2 className="text-xl font-bold text-white mb-2">No hay lecciones aún</h2>
