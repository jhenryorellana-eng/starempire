...
import type { ProfileStats, Reflection, StudentCategory } from '@/types';
...
export default function PerfilPage() {
  ...
  const strongestCategory = stats?.categories?.reduce(
    (best: StudentCategory | null, curr: StudentCategory) =>
      ...
      <div className="relative z-10 px-5 py-6 space-y-8">
        ...
        <div className="grid grid-cols-2 gap-3">
          <StatCard
            icon="domain"
            iconColor="#10B981"
            value={stats?.totalCompaniesExplored || 0}
            label="Empresas"
          />
          <StatCard
            icon="lightbulb"
        ...
        <div className="glass-card rounded-2xl p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-base font-bold text-white">Mi Perfil Empresarial</h3>
            <span className="material-icons-round text-gray-500 text-lg">info</span>
          </div>

          <div className="space-y-4">
            {CATEGORIES.map((cat) => {
              const userCat = stats?.categories?.find(
                (c) => c.categoryCode === cat.code
              );
              const score = userCat?.score || 0;
          ...
          {strongestCategory && (
            <p className="text-xs text-gray-400 mt-4 text-center">
              Tu categoría más fuerte es{' '}
              <span className="text-primary font-semibold">
                {CATEGORIES.find((c) => c.code === strongestCategory.categoryCode)?.name}
              </span>
            </p>
          )}
        </div>
        ...
        {reflections.length > 0 && (
          <div>
            <h3 className="text-base font-bold text-white mb-4">Mis Apuntes</h3>
                ...
                <div key={ref.id} className="glass-card rounded-xl p-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-gray-400">
                      {ref.companyTitle} • {ref.lessonTitle}
                    </span>
                  </div>
                </div>
              ))}
